...
import { isRef as isRef2, ref as ref2, watchEffect as watchEffect2, computed as computed2, inject as inject2, unref as unref2, watch as watch2, getCurrentInstance as getCurrentInstance2, customRef as customRef2, onUpdated, reactive as reactive2, shallowRef as shallowRef2, onMounted as onMounted2, markRaw, getCurrentScope as getCurrentScope2, isVue2 as isVue22, set as set2, del, readonly as readonly2, onBeforeUpdate } from "vue-demi";
...
function useDraggable(target, options = {}) {
  ...
  const position = ref2((_b2 = options.initialValue) != null ? _b2 : { x: 0, y: 0 });
  ...
  return __spreadProps$42(__spreadValues$b({}, toRefs(position)), {
    position,
...
function useFetch(url, ...args) {
  ...
  const execute = async (throwOnFailed = false) => {
    ...
    const context = { url: unref2(url), options: __spreadValues$9(__spreadValues$9({}, defaultFetchOptions), fetchOptions), cancel: () => {
      isCanceled = true;
    } };
...
function useFileSystemAccess(options = {}) {
  ...
  async function open(_options = {}) {
    if (!isSupported)
      return;
    const [handle] = await window2.showOpenFilePicker(__spreadValues$82(__spreadValues$82({}, unref2(options)), _options));
  ...
  async function create(_options = {}) {
    if (!isSupported)
      return;
    fileHandle.value = await window2.showSaveFilePicker(__spreadValues$82(__spreadValues$82({}, unref2(options)), _options));
    data.value = void 0;
    await updateFile();
    await updateData();
  }
...
function useInfiniteScroll(element, onLoadMore, options = {}) {
  ...
  watch2(() => state.arrivedState.bottom, (v) => {
    if (v)
      onLoadMore(state);
  });
}
...
function useUrlSearchParams(mode = "history", options = {}) {
  ...
  const state = reactive2(initialValue);
...
function useUserMedia(options = {}) {
  var _a2, _b2, _c;
  const enabled = ref2((_a2 = options.enabled) != null ? _a2 : false);
  const autoSwitch = ref2((_b2 = options.autoSwitch) != null ? _b2 : true);
...
function useVModel(props, key, emit, options = {}) {
  ...
  if (passive) {
    const proxy = ref2(props[key] || defaultValue);
